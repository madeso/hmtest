...
using namespace math;
		...
		void Quat::setEuler(const float p_x, const float p_y, const float p_z) {
		...
		void Quat::set(const float p_x, const float p_y, const float p_z, const float p_w) {
			m_x = p_x;
			m_y = p_y;
			m_z = p_z;
			m_w = p_w;
		...
		const float Quat::getX() const {
			return m_x;
		}
		const float Quat::getY() const {
			return m_y;
		}
		const float Quat::getZ() const {
			return m_z;
		}
		const float Quat::getW() const {
			return m_w;
		}
...
#define vecAsArray(v) {v.getX(), v.getY(), v.getZ()}
		...
		void Quat::fromMatrix3(const float mat[3][3])
		{
		  int   NXT[] = {1, 2, 0};
		  float q[4];
...
// http://www.faqs.org/faqs/graphics/algorithms-faq/ 
		...
		Quat Quat::slerp(const Quat& p_to, const float p_time) const {
