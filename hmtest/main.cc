...
void initOpenGL() {
	...
	if (ilGetInteger(IL_VERSION_NUM) < IL_VERSION) {
		cerr << "Devil version is different, compiled for: " << IL_VERSION << " driver: " << ilGetInteger(IL_VERSION_NUM);
	}
...
public:
	...
	virtual bool step(real pTime) = 0;
	virtual void onEvent( sgl::Event& pEvent) = 0;
};
...
public:
	...
	bool operator<(const ImageDescription& pOther) const {
...
public:
	...
	LoadedImage* getLoadedImage(const ImageDescription& pImageDesc);
	void unloadLoadedImage(const ImageDescription& pImageDesc, LoadedImage* pMedia);
private:
	...
	typedef std::map<ImageDescription, boost::shared_ptr<LoadedImage> > DescriptionImageMap;
	DescriptionImageMap mImages;
};

class Media {
public:
	Media(Engine* pEngine, bool pHardware) : loaded(false), hardware(pHardware), engine(pEngine) {
	...
	bool isHardwareDependent() const {
		return hardware;
	}
...
public:
	TemplateMedia(Engine* pEngine, bool pHardware) : Media(pEngine, pHardware), mMedia(0) {
	}
	virtual ~TemplateMedia() {
	}
	const Type* getMedia() const {
		Assert(isLoaded(), "Media isn't loaded, call load before use");
		Assert(mMedia, "Doesn't have a valid type, but the media is loaded - bug?");
		return mMedia;
	}
protected:
	Type* mMedia;
...
LoadedImage* Engine::getLoadedImage(const ImageDescription& pImageDesc) {
...
void Engine::unloadLoadedImage(const ImageDescription& pImageDesc, LoadedImage* pMedia) {
	mImages.erase(mImages.find(pImageDesc));
}

void Engine::unloadHardware() {
	mLoadState = LS_UNLOADING;
	for(MediaList::iterator mediaIterator=mMediaList.begin(); mediaIterator != mMediaList.end(); ++mediaIterator) {
...
void Engine::processOne() {
	...
	if( mLoadState == LS_LOADING) {
		...
		Assert(mLoadState == LS_UNDEFINED, "Unknown value, state may be valid but unimplemented, consult programer");
		Assert(mLoadState != LS_UNDEFINED, "Don't know what to do, called to often?");
	}
...
public:
	Image(Engine* pEngine, const ImageDescription& pImageDesc) : TemplateMedia(pEngine, true), mImageDesc(pImageDesc) {
	}
	~Image() {
	}
	...
	const LoadedImage* operator->() const {
		return getMedia();
	}
private:
...
public:
	...
	// per primitive
	unsigned int getUniform(const std::string& pName) {
		unsigned int id = glGetUniformLocationARB(program, pName.c_str());
		return id;
	}

	// per vetex
	unsigned int getAttribute(const std::string& pName) {
...
protected:
	void checkShaderCompilation(unsigned int shader, const std::string& shaderName) {
		int status=0;
		glGetObjectParameterivARB(shader, GL_OBJECT_COMPILE_STATUS_ARB, &status);
		if( status != 1 ) {
			std::ostringstream message;
			message << "Error when compiling: " << shaderName << ": " << getInfoLog(shader);
			throw message.str();
		}
	}
	void checkProgram(const std::string& pVertex, const std::string& pFrag) {
		int status=0;
		glGetObjectParameterivARB(program, GL_OBJECT_LINK_STATUS_ARB, &status);
		if( status != 1 ) {
			std::ostringstream message;
			message <<  "Error when linking program(" << pVertex << ", " << pFrag <<  "): " << getInfoLog(program);
			throw message.str();
		}
	}
	const std::string getInfoLog(unsigned int object) {
		int len = 0;
...
class Cube {
public:
	Cube(Engine* pEngine, const vec3& pPos, const std::string& pTexture) : pos(pPos), image(pEngine, ImageDescription(pTexture)) {
	}
	void render(bool applyTexture) {
...
struct Attenuation {
	...
	void operator()(float c, float l, float q) {
		constant = c;
		linear = l;
		quadratic = q;
	}
...
CommonLightAttributes::CommonLightAttributes() : ambient(1.0f,1.0f,1.0f), diffuse(1.0f,1.0f,1.0f), specular(0.0f,0.0f,0.0f) {
}
Attenuation::Attenuation() : constant(0.1f), linear(0.1f), quadratic(0.005f) {
}

struct Material {
	Material() : mAmbient(0.2f, 0.2f, 0.2f), mDiffuse(0.8f, 0.8f, 0.8f), mSpecular(1.0f, 1.0f, 1.0f), mEmissive(0.0f, 0.0f, 0.0f), mShininess(100.0f), mAlpha(1.0f) {
	}
	void sendToGl() {
...
struct Light : public CommonLightAttributes {
	Light() : spot(false), position(0,0,0), direction(0,0,-1), exponent(128), cutoff(30) {
	}
...
Index toIndex(const std::string& str) {
...
public:
	explicit Mesh(const std::string& pFile) {
		...
		while( std::getline(f, line) ) {
			...
			if( commands.size() > 0 ) {
				...
				if( type == "v" ) {
					vertices.push_back( vec3(toReal(commands[1]), toReal(commands[2]), toReal(commands[3])) );
				}
				else if (type == "vn" ) {
					normals.push_back( vec3(toReal(commands[1]), toReal(commands[2]), toReal(commands[3])) );
				}
				else if( type == "vt" ) {
					tex.push_back( vec2(toReal(commands[1]), toReal(commands[2])) );
				}
				else if( type == "f" ) {
					faces.push_back( Face(	toIndex(commands[1]),
											toIndex(commands[2]),
											toIndex(commands[3])
											) );
				}
	...
	void render() const {
		...
		for( std::list<Face>::const_iterator face=faces.begin(); face!=faces.end(); ++face) {
			renderFace(*face);
		}
		glEnd();
	}
...
class World {
public:
	explicit World(Engine* pEngine) : waterDisp(0), px(10), py(5), pz(20), nx(-10), ny(-1.1), wy(-2), by(-5.0), nz(-5), zs(10), ze(17), wall(pEngine, ImageDescription("images/wand.bmp")), water(pEngine, ImageDescription("images/water512.jpg")), spotImage(pEngine, ImageDescription("images/spot.png")),
							hasSpot(false), spotLocation(0, 0, 0), spotDirection(0, 0, -1),
	...
	void setSpot(const vec3& pos, const vec3& dir) {
		spotLocation = pos;
		spotDirection = dir;
	}
	void setSpot(bool enable) {
		hasSpot = enable;
	}
protected:
	...
	void setupOpenglLight(const Light& light) {
		if( light.spot ) {
		...
		glLightf(GL_LIGHT0, GL_CONSTANT_ATTENUATION, light.attenuation.constant);
		glLightf(GL_LIGHT0, GL_LINEAR_ATTENUATION, light.attenuation.linear);
		glLightf(GL_LIGHT0, GL_QUADRATIC_ATTENUATION, light.attenuation.quadratic);

#pragma warning(disable:4244)
		float directionArray[3] = { light.direction.getX(), light.direction.getY(), light.direction.getZ() };
		glLightfv(GL_LIGHT0, GL_SPOT_DIRECTION, directionArray);

		float positionArray[4] = { light.position.getX(), light.position.getY(), light.position.getZ(), 1.0f };
		glLightfv(GL_LIGHT0, GL_POSITION, positionArray);

		float ambientArray[4] = { light.ambient.getX(), light.ambient.getY(), light.ambient.getZ(), 1.0f };
		glLightfv(GL_LIGHT0, GL_AMBIENT, ambientArray);

		float diffuseArray[4] = { light.diffuse.getX(), light.diffuse.getY(), light.diffuse.getZ(), 1.0f };
		glLightfv(GL_LIGHT0, GL_DIFFUSE, diffuseArray);

		float specularArray[4] = { light.specular.getX(), light.specular.getY(), light.specular.getZ(), 1.0f };
...
#pragma warning(default:4244)
	...
	void doLight(const Light& light) {
		glDisable(GL_BLEND);
		...
		if( light.spot ) {
			...
			unsigned int spotTexture = shaderSpotLight.getUniform("spotTexture");
	...
	void drawScene(bool applyTexture, bool doTransparent) {
		Material().sendToGl();
		if(applyTexture) glBindTexture(GL_TEXTURE_2D, wall->getId());
		mesh.render();
	}
	void drawScene_old(bool applyTexture, bool doTransparent) {
		Material().sendToGl();
		...
		glBegin(GL_QUADS);
			// Top Face
			glNormal3f( 0.0f, -1.0f, 0.0f);
			glTexCoord2d(0.0, (pz-nz)/4); glVertex3d(nx, py, nz);	// Top Left Of The Texture and Quad
			glTexCoord2d(0.0, 0.0); glVertex3d(nx, py, pz);	// Bottom Left Of The Texture and Quad
			glTexCoord2d((px-nx)/4, 0.0); glVertex3d(px, py, pz);	// Bottom Right Of The Texture and Quad
			glTexCoord2d((px-nx)/4, (pz-nz)/4); glVertex3d(px, py, nz);	// Top Right Of The Texture and Quad
		glEnd();
		...
		glBegin(GL_QUADS);
			...
			// Bottom Face -start
			glTexCoord2d((px-nx)/4, (zs-nz)/4); glVertex3d(nx, ny, nz);	// Top Right Of The Texture and Quad
			glTexCoord2d(0.0, (zs-nz)/4); glVertex3d(px, ny, nz);	// Top Left Of The Texture and Quad
			glTexCoord2d(0.0, 0.0); glVertex3d(px, ny, zs);	// Bottom Left Of The Texture and Quad
			glTexCoord2d((px-nx)/4, 0.0); glVertex3d(nx, ny, zs);	// Bottom Right Of The Texture and Quad

			// Bottom Face - end
			glTexCoord2d((px-nx)/4, (pz-ze)/4); glVertex3d(nx, ny, ze);	// Top Right Of The Texture and Quad
			glTexCoord2d(0.0, (pz-ze)/4); glVertex3d(px, ny, ze);	// Top Left Of The Texture and Quad
			glTexCoord2d(0.0, 0.0); glVertex3d(px, ny, pz);	// Bottom Left Of The Texture and Quad
			glTexCoord2d((px-nx)/4, 0.0); glVertex3d(nx, ny, pz);	// Bottom Right Of The Texture and Quad
		glEnd();

		glBegin(GL_QUADS);
			// Right face
			glNormal3f(-1.0f, 0.0f, 0.0f);	
			glTexCoord2d((ze-zs)/4, 0.0); glVertex3d(px, by, zs);	// Bottom Right Of The Texture and Quad
			glTexCoord2d((ze-zs)/4, (ny-by)/4); glVertex3d(px, ny, zs);	// Top Right Of The Texture and Quad
			glTexCoord2d(0.0, (ny-by)/4); glVertex3d(px, ny, ze);	// Top Left Of The Texture and Quad
			glTexCoord2d(0.0, 0.0); glVertex3d(px, by, ze);	// Bottom Left Of The Texture and Quad
			// Left Face
			glNormal3f(1.0f, 0.0f, 0.0f);
			glTexCoord2d(0.0, 0.0); glVertex3d(nx, by, zs);	// Bottom Left Of The Texture and Quad
			glTexCoord2d((ze-zs)/4, 0.0); glVertex3d(nx, by, ze);	// Bottom Right Of The Texture and Quad
			glTexCoord2d((ze-zs)/4, (ny-by)/4); glVertex3d(nx, ny, ze);	// Top Right Of The Texture and Quad
			glTexCoord2d(0.0, (ny-by)/4); glVertex3d(nx, ny, zs);	// Top Left Of The Texture and Quad
			// back Face
			glNormal3f( 0.0f, 0.0f, -1.0f);
			glTexCoord2d(0.0, 0.0); glVertex3d(nx, by, ze);	// Bottom Left Of The Texture and Quad
			glTexCoord2d((px-nx)/4, 0.0); glVertex3d(px, by, ze);	// Bottom Right Of The Texture and Quad
			glTexCoord2d((px-nx)/4, (ny-by)/4); glVertex3d(px, ny, ze);	// Top Right Of The Texture and Quad
			glTexCoord2d(0.0, (ny-by)/4); glVertex3d(nx, ny, ze);	// Top Left Of The Texture and Quad
			// front Face
			glNormal3f( 0.0f, 0.0f, 1.0f);
			glTexCoord2d((px-nx)/4, 0.0); glVertex3d(nx, by, zs);	// Bottom Right Of The Texture and Quad
			glTexCoord2d((px-nx)/4, (ny-by)/4); glVertex3d(nx, ny, zs);	// Top Right Of The Texture and Quad
			glTexCoord2d(0.0, (ny-by)/4); glVertex3d(px, ny, zs);	// Top Left Of The Texture and Quad
			glTexCoord2d(0.0, 0.0); glVertex3d(px, by, zs);	// Bottom Left Of The Texture and Quad
		glEnd();
		glBegin(GL_QUADS);
			// Bottom Face
			glNormal3f( 0.0f, 1.0f, 0.0f);
			glTexCoord2d((px-nx)/4, (ze-zs)/4); glVertex3d(nx, by, zs);	// Top Right Of The Texture and Quad
			glTexCoord2d(0.0, (ze-zs)/4); glVertex3d(px, by, zs);	// Top Left Of The Texture and Quad
			glTexCoord2d(0.0, 0.0); glVertex3d(px, by, ze);	// Bottom Left Of The Texture and Quad
			glTexCoord2d((px-nx)/4, 0.0); glVertex3d(nx, by, ze);	// Bottom Right Of The Texture and Quad
		glEnd();

		glBegin(GL_QUADS);
			// Right face
			glNormal3f( -1.0f, 0.0f, 0.0f);
			glTexCoord2d((pz-nz)/4, 0.0); glVertex3d(px, ny, nz);	// Bottom Right Of The Texture and Quad
			glTexCoord2d((pz-nz)/4, (py-ny)/4); glVertex3d(px, py, nz);	// Top Right Of The Texture and Quad
			glTexCoord2d(0.0, (py-ny)/4); glVertex3d(px, py, pz);	// Top Left Of The Texture and Quad
			glTexCoord2d(0.0, 0.0); glVertex3d(px, ny, pz);	// Bottom Left Of The Texture and Quad
			// Left Face
			glNormal3f( 1.0f, 0.0f, 0.0f);
			glTexCoord2d(0.0, 0.0); glVertex3d(nx, ny, nz);	// Bottom Left Of The Texture and Quad
			glTexCoord2d((pz-nz)/4, 0.0); glVertex3d(nx, ny, pz);	// Bottom Right Of The Texture and Quad
			glTexCoord2d((pz-nz)/4, (py-ny)/4); glVertex3d(nx, py, pz);	// Top Right Of The Texture and Quad
			glTexCoord2d(0.0, (py-ny)/4); glVertex3d(nx, py, nz);	// Top Left Of The Texture and Quad
			// back Face
			glNormal3f( 0.0f, 0.0f, -1.0f);
			glTexCoord2d(0.0, 0.0); glVertex3d(nx, ny, pz);	// Bottom Left Of The Texture and Quad
			glTexCoord2d((px-nx)/4, 0.0); glVertex3d(px, ny, pz);	// Bottom Right Of The Texture and Quad
			glTexCoord2d((px-nx)/4, (py-ny)/4); glVertex3d(px, py, pz);	// Top Right Of The Texture and Quad
			glTexCoord2d(0.0, (py-ny)/4); glVertex3d(nx, py, pz);	// Top Left Of The Texture and Quad
			// front Face
			glNormal3f( 0.0f, 0.0f, 1.0f);	
			glTexCoord2d((px-nx)/4, 0.0); glVertex3d(nx, ny, nz);	// Bottom Right Of The Texture and Quad
			glTexCoord2d((px-nx)/4, (py-ny)/4); glVertex3d(nx, py, nz);	// Top Right Of The Texture and Quad
			glTexCoord2d(0.0, (py-ny)/4); glVertex3d(px, py, nz);	// Top Left Of The Texture and Quad
			glTexCoord2d(0.0, 0.0); glVertex3d(px, ny, nz);	// Bottom Left Of The Texture and Quad
		glEnd();
		...
		if( false && doTransparent ) {
			...
			glMatrixMode(GL_TEXTURE);
				glLoadIdentity();
				glTranslated(0.25*math::sin(waterDisp * 2*math::PI), 0.25*math::cos(waterDisp * 2*math::PI), 0);
			glMatrixMode(GL_MODELVIEW);
			glBegin(GL_QUADS);
				// Bottom Face -start
				glNormal3f( 0.0f, 1.0f, 0.0f);
				glTexCoord2d((px-nx)/4, (ze-zs)/4); glVertex3d(nx, wy, zs);	// Top Right Of The Texture and Quad
				glTexCoord2d(0.0, (ze-zs)/4); glVertex3d(px, wy, zs);	// Top Left Of The Texture and Quad
				glTexCoord2d(0.0, 0.0); glVertex3d(px, wy, ze);	// Bottom Left Of The Texture and Quad
				glTexCoord2d((px-nx)/4, 0.0); glVertex3d(nx, wy, ze);	// Bottom Right Of The Texture and Quad
			glEnd();
			// restore matrix
			glMatrixMode(GL_TEXTURE);
				glLoadIdentity();
...
public:
	Game(Engine* pEngine) :	mCameraPosition(0, 0, 0),
							...
							shader("shaders/simple.vert", "shaders/simple.frag"), light(false) {
	}
	void render() {
		glColor4f(1, 1, 1,1);
	...
	void increaseRotation(const Quat& pRot) {
		mCameraRotation = pRot
						* mCameraRotation ;
	}
	void onEvent( sgl::Event& pEvent) {
		switch( pEvent.type ) {
			...
			case sgl::EVENT_KEYUP:
				{
					...
					switch( pEvent.k ) {
						...
						case sgl::Key::Space:
							up = isDown;
							break;
						case sgl::Key::Mouse_Right:
							grid.setEnable(isDown);
							break;
...
public:
	Loading(Engine* pEngine) : mEngine(pEngine), mLoaded(0), mToLoad(0), mProgress(0), mTime(0) {
		mEngine->loadMedia();
	...
	void render() {
		...
		glBegin(GL_QUADS);
			rectangle(0.1, 0.8, 0.8,
				0.02);
			if( mLoaded > 0 ) {
				glColor3d(0.4, 0.4, 0.4);
				rectangle(0.1, 0.8, 0.8 * mProgress,
					0.02);
			}
		glEnd();
		glEnable(GL_TEXTURE_2D);
...
void SGL_main(const std::string& arg0) {
	...
	if( FULLSCREEN ) {
		sgl::SetVideoMode(sgl::VideoMode().setResolution(gWidth, gHeight).setFullscreen() );
	}
	else {
		sgl::SetVideoMode(sgl::VideoMode().setResolution(gWidth, gHeight).setWindowed() );
	}
	initOpenGL();
